...
MODEL_PATH = os.path.join(package_share, "models", "0306transformer_action_recognition_holistic6060.pth")
...
#     swapped_probs = np.copy(probs)

    
#     swapped_probs[1], swapped_probs[3] = probs[3], probs[1]

    
#     swapped_probs[9], swapped_probs[4] = probs[4], probs[9]

#     output_frame = input_frame.copy()
#     limit = min(len(swapped_probs), len(actions), len(colors))
#     for i in range(limit):
#         action = actions[i]
#         c = colors[i]
#         # Use the swapped probability
#         prob = swapped_probs[i]
#         prob_percent = int(prob * 100)

#         # Draw bar
#         start_point = (0, 60 + i * 40)
#         end_point = (prob_percent, 90 + i * 40)
#         cv2.rectangle(output_frame, start_point, end_point, c, -1)

#         # Label text: e.g. "DESCEND 45%"
...
def prob_viz(probs, actions, input_frame, colors):
    ...
    for i in range(limit):
        ...
        cv2.rectangle(output_frame, start_point, end_point, c, -1)
        label_text = f"{action} {prob_percent}%"
        cv2.putText(output_frame, label_text, (0, 85 + i * 40),
...
def extract_landmarks(results):
    """
    Extract pose (33x3), left-hand (21x3) and right-hand (21x3)
    landmarks into a 225-dim vector.
    """
...
class ActionRecognitionNode(Node):
    def __init__(self):
        ...
        self.model = ActionTransformer(
            feature_dim=225,   # 33 pose x 3 + 21 left-hand x 3 + 21 right-hand x 3
