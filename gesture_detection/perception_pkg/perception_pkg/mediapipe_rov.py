...
def is_buddy_up(rov, action_duration_sec=2.0):
    ...
    while time.time() - start_time < action_duration_sec:
        ...
        if hand_results["Left"] == "Thumb_Up" and hand_results["Right"] == "Thumb_Up":
            buddy_up_frames += 1
        total_frames += 1
...
def get_body_frame(rov):
    ...
    center_of_mass = np.mean(
        [left_shoulder, right_shoulder, left_hip, right_hip], axis=0
    )
...
class ROVAction:
    ...
    def follow_diver_action(bbox, frame_width, frame_height):
        ...
        print(
            f"[FOLLOW] Bounding box center: ({mean_x:.2f}, {mean_y:.2f}), Offsets: (dx={dx:.2f}, dy={dy:.2f})"
        )
    ...
    def turn_action(rov):
        ...
        if (
            not hand_gestures
            or "Left" not in hand_gestures
            or "Right" not in hand_gestures
        ):
        ...
        if left_hand_gesture == "Closed_Fist" and right_hand_gesture == "Open_Palm":
            ...
            and right_hand_gesture == "Unknown"
            or right_hand_gesture == "Open_Palm"
            and left_hand_gesture == "Unknown"
        ):
